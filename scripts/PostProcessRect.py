...
def PostProcess(inputfile, outputfile):
	...
	for element in elements:
		draw.polygon([Transform(nodes[element[0]]), Transform(nodes[element[1]]), Transform(nodes[element[2]]), Transform(nodes[element[3]])], fill=0x20a020, outline=0x707020)
	...
	for idx,element in enumerate(elements):
		...
		draw.polygon(
			[
				...
				Transform(getNodePosition(element[3], nodes, deforms))
			], 
			fill = colori
		)
